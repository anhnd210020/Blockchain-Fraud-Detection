...
def run_script(script_name):
    try:
        script_path = os.path.join("Dataset", script_name)
        print(f"Running {script_path}...")
        subprocess.run(['python', script_path], check=True, env=dict(os.environ, PYTHONPATH='.'))
...
def move_files_to_preprocessed_folder():
    ...
    for file_name in source_files:
        if os.path.exists(file_name):
            dst_path = os.path.join(destination_folder, os.path.basename(file_name))
